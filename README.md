...
安装脚本会在 `~/Applications/CDP Auto Allow.app` 构建并 ad-hoc 签名应用，在 `~/Library/LaunchAgents/` 安装并启动 LaunchAgent。首次安装后，到“系统设置 → 隐私与安全性 → 辅助功能”中添加并启用该应用：

1. 点击添加按钮，按 `⌘⇧G` 输入 `~/Applications/CDP Auto Allow.app`。
2. 打开 `CDP Auto Allow` 的辅助功能开关。
3. 确认服务已启动：

   ```sh
   launchctl print "gui/$(id -u)/com.local.cdp-auto-allow"
   ```
...
```sh
./scripts/dry-run.sh
tail -f /tmp/cdp-auto-allow.debug.log
```
...
可在运行安装脚本前设置环境变量；安装脚本会把值写入 LaunchAgent：

| 变量 | 默认值 | 作用 |
| --- | --- | --- |
| `CDP_AUTO_ALLOW_DRY_RUN` | `0` | 设为 `1` 时只记录将点击的按钮，不执行点击 |
| `CDP_AUTO_ALLOW_FOCUS_STEAL` | `0` | 设为 `1` 时，只有无焦点直接点击失败后才激活 Chrome 并重试明确匹配到的允许按钮 |
| `CDP_AUTO_ALLOW_INTERVAL` | `0.4` | 轮询间隔，范围为 0.2–5 秒 |
| `CDP_AUTO_ALLOW_APP_PATH` | `~/Applications/CDP Auto Allow.app` | 自定义应用安装位置 |

例如，启用常驻 dry-run：

```sh
CDP_AUTO_ALLOW_DRY_RUN=1 ./scripts/install.sh
```

改回实际点击模式：

```sh
CDP_AUTO_ALLOW_DRY_RUN=0 ./scripts/install.sh
```
...
- 日志：`/tmp/cdp-auto-allow.debug.log`
- 服务状态：`launchctl print "gui/$(id -u)/com.local.cdp-auto-allow"`
- 监听测试：`./scripts/dry-run.sh`
- 授权失效：确认辅助功能中启用的是当前的 `CDP Auto Allow.app`；应用重新编译后可能需要移除旧条目并重新添加。
...
plutil -lint launchd/com.local.cdp-auto-allow.plist
```
